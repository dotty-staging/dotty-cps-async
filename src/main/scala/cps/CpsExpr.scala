...
trait CpsExpr[F[_]:Type,T:Type](monad:Expr[CpsMonad[F]], prev: Seq[ExprTreeGen]):
  ...
  def transformed(using Quotes): Expr[F[T]] =
     ...
     if (prev.isEmpty)
       ...
       Block(prev.toList.map(_.extract), Term.of(fLast)).asExpr.cast[F[T]]
  ...
  //def pure[A:Type](t: Expr[A])(using Quotes): CpsExpr[F,A] =
...
abstract class SyncCpsExpr[F[_]:Type, T: Type](dm: Expr[CpsMonad[F]],
     ...
     override def syncOrigin(using Quotes): Option[Expr[T]] = Some(
         if prev.isEmpty then
            ...
            quotes.reflect.Block(prev.toList.map(_.extract), Term.of(last)).asExpr.cast[T]
     )
