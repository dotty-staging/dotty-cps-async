    ...
    tpe.widen match {
      case MethodType(paramNames, paramTypes, resType) =>
               ...
               MethodType(paramNames)( mt => paramTypes,
                                       mt => fType.asReflectTree.tpe.appliedTo(resType))
      case PolyType(paramNames,paramBounds,resType) =>
               PolyType(paramNames)(pt => paramBounds,
                                    pt => fType.asReflectTree.tpe.appliedTo(resType))
       ...
       findObjectAsyncShiftTerm(qual) match
         case success1: ImplicitSearchSuccess =>
           ...
           val shiftedExpr = Apply(
                               TypeApply(Select.unique(objectShift, "apply"),fType.asReflectTree::Nil),
                               List(qual,monad)
                             )
             ...
             case success2: ImplicitSearchSuccess =>
               ...
               shiftSymbol.method(x.name) match
                    ...
                    case m::Nil =>
                        ...
                        TypeApply(newSelect, fType.asReflectTree::targs).appliedTo(qual,monad)
                    case other =>
                        Select.overloaded(success2.tree, x.name, (fType.asReflectTree::targs).map(_.tpe), List(qual,monad))
    ...
    def shiftCaller(term:Term): Term =
       ...
       term match
          ...
          case s@Select(qual,name) =>
                  //TypeApply(shiftSelect(s), fType.asReflectTree::Nil).appliedTo(qual, monad)
