...
class TryTransform[F[_]:Type,T:Type](cpsCtx: TransformationContext[F,T]):
  ...
  def run(using Quotes)(body: qctx.reflect.Term,
     ...
     def makeRestoreExpr(): Expr[Throwable => F[T]]  =
        val nCaseDefs = (cases lazyZip cpsCaseDefs) map { (frs,snd) =>
           CaseDef(frs.pattern, frs.guard, snd.Term.of(transformed))
        }
        val restoreExpr = '{ (ex: Throwable) => ${Match('Term.of(ex), nCaseDefs.toList).asExprOf[F[T]]} }
     ...
     val builder = if (!isAsync) {
                      ...
                      val errorMonad = if (Term.of(monad).tpe <:< TypeRepr.of[CpsTryMonad[F]]) {
