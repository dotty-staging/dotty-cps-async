...
trait MM[F[_]] {
  ...
  def mmTransform[T:Type](expr: Expr[T])(using qctx: Quotes):Expr[F[T]]

}

implicit object MMX extends MM[X] {

  def mmTransform[T:Type](expr: Expr[T])(using qctx: Quotes):Expr[X[T]] =
   '{ new X{ val v=${expr} } }
...
object A3 {
  ...
  def aTransform[F[_]:Type,T:Type](e:Expr[T])(using qctx: Quotes):Expr[F[T]] =
